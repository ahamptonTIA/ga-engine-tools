...
def set_pyspark_schema_from_feature_service(
	...
	"""
	Cast a Spark DataFrame to match an ArcGIS Feature Service schema.
	"""
	geom_types = [
	...
	try:
		...
		# Mapping Esri field types to Spark SQL types
		type_map = {
		...
		# Add remaining columns not in the service schema
		select_list += [
		...
		_logger.error(f"Failed to cast schema: {e}")
		raise
...
def _feature_service_log(
	...
	if log_table:
		...
		try:
			...
			_logger.error(f"Failed to write log to {log_table}: {e}")

	print(f"Batch '{batch_id}' {status.lower()}: {log_msg} (Attempt {attempt})")
...
def check_create_feature_service(
	...
	"""
	Check if a Feature Service exists or optionally create a new one.
	"""
	try:
		...
		_logger.error(f"Service check/creation error: {e}")
		return False, None, None, []
...
def write_df_in_chunks(
	...
	"""
	Write Spark DataFrame to a Feature Service using row-based chunking.
	"""
	...
	for idx in range(1, max_idx + 1):
		...
		# Only the very first chunk of the first batch respects 'overwrite'
...
def update_arcgis_feature_service(
	...
	"""
	Orchestrate the update of an ArcGIS Feature Service from Spark.
	"""
	# Initialize GIS
	...
	while attempt <= max_retries:
		...
		batches = [
			row[0] for row in input_df.select(batch_col).distinct().collect()
		]
		
		try:
			# Use executor to handle potential timeouts
			with ThreadPoolExecutor(max_workers=1) as executor:
				...
				for b_id in batches:
					...
					future = executor.submit(
						...
						parallelism, svc_url, layer_name, retry_delay,
						attempt, max_chunk
					)
					...
					# Update flags after successful first batch
					_, is_first, _ = res
